<template>
  <div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Add Coffee Process</h5>
            <button
              type="button"
              class="btn-close"
    ...
    <table
      ...
      v-for="coffeeProcess in coffeeProcessList"
      :key="coffeeProcess._id"
    >
      <tr>
        <th>
          <img
        ...
        <td>
          {{ coffeeProcess.fileName }}<br />
          {{ coffeeProcess.description }}<br /><br />
          <router-link
            :to="{ name: 'EditCoffee', params: { id: coffeeProcess._id } }"
            class="edit"
            ><!-- EDIT ICON -->
            <fa icon="edit" />
          </router-link>
          <a href="" @click.prevent="deleteCoffeeProcess(coffeeProcess._id)"
            ><!-- DELETE ICON -->
...
export default {
  ...
  created() {
    const endpoint = '/coffee-processes';
    axios
      .get(api + endpoint)
      .then((res) => {
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      try {
        ...
        await axios.post(api + endpoint2, this.coffeeProcess);
        this.coffeeProcess = {
          filename: '',
          description: '',
        };
    ...
    // DELETE OPERATION
    async deleteCoffeeProcess(id) {
      const endpoint = `/delete-coffee-process/${id}`;
      try {
        await axios.get(api + endpoint).then(() => {
          // eslint-disable-next-line no-underscore-dangle
          this.coffeeProcessList.splice(
            // eslint-disable-next-line no-underscore-dangle
            this.coffeeProcessList.findIndex((i) => i._id === id),
            1,
          );
        });
        this.message = 'Coffee Process deleted successfully.';
      } catch {
